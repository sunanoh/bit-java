...
public class DateEx {
	...
	public static void main(String[] args) {
		...
		// Deprecated된 메서드는 사라질 메서드 -> 다른 방식으로 교체가 필요함
		...
		System.out.println("LONG 포맷 : "+df2.format(now));
		
		DateFormat df3 = DateFormat.getDateInstance(DateFormat.MEDIUM);
		System.out.println("MEDIUM 포맷 : "+df3.format(now));
		
		DateFormat df4 = DateFormat.getDateInstance(DateFormat.SHORT);
		System.out.println("SHORT 포맷 : "+df4.format(now));
		
		// 자유로운 날짜 데이터 포매팅
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy년 MM월 dd일 HH시 mm분 ss초");
		System.out.println("자유 포맷 : "+ sdf.format(now));
	}
